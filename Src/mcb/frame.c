...
IER_RET FrameCreate(TFrame* tFrame, TFrameType tFrameType, uint16_t u16Node,
                    uint16_t u16SubNode, uint16_t u16Addr, uint8_t u8Cmd,
                    uint8_t u8Pending, const void* pStaBuf, const void* pDynBuf,
                    size_t szDyn, bool calcCRC)
{
    ...
    while (1)
    {
        ...
        if (pStaBuf != NULL)
        {
            memcpy(&tFrame->buf[u16HeadSz], pStaBuf,
                  (sizeof(tFrame->buf[0]) * HSP_FRM_STA_SZ));
        }
        else
        {
            memset(&tFrame->buf[u16HeadSz], 0,
                  (sizeof(tFrame->buf[0]) * HSP_FRM_STA_SZ));
        }
...
uint16_t FrameGetNode(const TFrame* tFrame)
{
    TNodeHeader tNodeHeader;
...
uint16_t FrameGetSubNode(const TFrame* tFrame)
{
    TNodeHeader tNodeHeader;
    if (tFrame->tFrameType == UART_FRAME)
    {
...
bool FrameGetSegmented(const TFrame* tFrame)
{
    THeader tHeader;
...
uint16_t FrameGetAddr(const TFrame* tFrame)
{
    THeader tHeader;
    uint16_t u16HeadField = 0;
    if (tFrame->tFrameType == UART_FRAME)
...
uint8_t FrameGetCmd(const TFrame* tFrame)
{
    THeader tHeader;
    uint16_t u16HeadField = 0;
    if (tFrame->tFrameType == UART_FRAME)
...
uint16_t FrameGetStaticData(const TFrame* tFrame, uint16_t* buf)
{
    uint16_t u16HeadField = 0;
    if (tFrame->tFrameType == UART_FRAME)
    {
